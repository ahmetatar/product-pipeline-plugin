...
- **Local-simulator branches may carry unpushed In-Test fix commits.** In `local-simulator` mode `dev-story-implementer` does NOT re-push each In-Test fix (CI there is build-only — re-pushing every tweak adds no signal); the fixes are committed locally and relaunched on the simulator. Step 7's `git push` is what delivers that accumulated work to the PR — so it's normal for `git log origin/$BRANCH..$BRANCH` to be non-empty here, and step 6's green check may reflect the pre-fix HEAD. Step 7's `gh pr checks --watch` (after the push) is the authoritative gate; the squash-merge collapses the intermediate fix commits into one. (Testflight branches are already fully pushed — each fix re-pushed during In-Test.)
